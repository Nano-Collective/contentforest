...
**Punctuation:** use regular hyphens (`-`), never em-dashes (`—`). Em-dashes signal AI-generated copy. Use a comma, parenthesis, colon, or full stop instead. En-dashes (`–`) are fine for numeric ranges only. **The validator fails on every em-dash.** One em-dash anywhere in your output blocks the pack.
