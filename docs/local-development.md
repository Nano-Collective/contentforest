...
- **Nanocoder TTY requirement.** Nanocoder is built on Ink and requires a raw-mode TTY even in `run` mode. Locally this is fine. In CI the orchestrator wraps the spawn in `script(1)` to allocate a pty (see `runNanocoder` in `scripts/generate-content.ts`). If you `pnpm generate` from inside a process without a TTY (e.g. piping into a file), you'll hit "Raw mode is not supported" — just run from a real terminal.
