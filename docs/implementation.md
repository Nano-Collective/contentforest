...
**Handoff state.** Pipeline is end-to-end working as of 2026-04-30. Drip-articles design folded into the release pack. Personal-content rendering bug fixed. Next agent / contributor should read [`handoff.md`](./handoff.md) first — it has prioritised TBDs covering operational unblock (PR-creation perms, team voice notes, first PR + 3-product backfill), the homepage redesign with three.js hero matching the main website, surfacing GitHub identity in the UI via Cloudflare Access's `/cdn-cgi/access/get-identity`, and the full doc set (README rewrite, runbook, local-dev, adding-a-product, LICENSE, PR template).
