...
| 2026-04-30 | **Orchestrator pre-writes `nanocoder-preferences.json` with cwd added to `trustedDirectories`.** | Nanocoder shows a "Do you trust this folder?" prompt on every fresh invocation in an unknown directory. Yolo mode doesn't bypass it. Project-level prefs file overrides user-level; orchestrator now writes it idempotently before each `runNanocoder` call. File is gitignored. This was the actual cause of the second CI hang (visible after `script -f` flushed Ink output to the log). |
